...
const Accordion = ({ i, expanded, setExpanded, titulo, id, texto }) => {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(AccHeading, { initial: false, animate: { backgroundColor: isOpen ? "transparent" : "transparent" }, onClick: () => setExpanded(isOpen ? false : i) },
            titulo,
            React.createElement("img", { src: ArrowAcc, 
                // initial={false}
                className: isOpen ? 'arrow-acc active' : 'arrow-acc' })),
        React.createElement(AnimatePresence, { initial: false }, isOpen && (React.createElement(motion.section, { key: "content", initial: "collapsed", animate: "open", exit: "collapsed", variants: {
                open: { opacity: 1, height: "auto" },
                collapsed: { opacity: 0, height: 0, overflow: 'hidden' }
            }, transition: { duration: 0.5, ease: [0.04, 0.62, 0.23, 0.98] } },
            React.createElement(AccordionItem, { texto: texto, id: id }))))));
};
export const ContainerAccordion = () => {
    // This approach is if you only want max one section open at a time. If you want multiple
    // sections to potentially be open simultaneously, they can all be given their own `useState`.
    const [expanded, setExpanded] = useState(false);
    return objectAccordion.map(i => (React.createElement(Accordion, { i: i, expanded: expanded, setExpanded: setExpanded, titulo: i.titulo, id: i.id, texto: i.texto, key: i.id })));
};
...
//# sourceMappingURL=ContainerAccordion.js.map